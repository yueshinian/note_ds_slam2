...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp)
{
    ...
    while(!isNewSegmentImgArrived()) //等待分割成功
    {
        usleep(1);
    }
    ...
    //优化图像
    cv::Mat imageSeg = maSegment->mImgSegmentLatest.clone();
    cv::Mat structureElement = cv::getStructuringElement(MORPH_ELLIPSE, Size(7, 7), Point(-1, -1));
    cv::erode(src, dst, structureElement);//腐蚀去噪？掩膜噪声
    cv::dilate(imageSeg, imageSeg, );//膨胀扩充分割边界？提取的特征点会落到掩膜边缘
