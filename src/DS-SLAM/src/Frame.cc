...
{ 
    ...
    //对极约束检测到动态点，语义分割有动态区域
    if(!T_M.empty() && flagprocess )
    {
        std::chrono::steady_clock::time_point tc1 = std::chrono::steady_clock::now();
        flag_mov = mpORBextractorLeft->CheckMovingKeyPoints(imGray,imS,mvKeysTemp,T_M);//std::vector<std::vector<cv::KeyPoint>> mvKeysTemp;
    ...
    ExtractORBDesp(0,imGray);//计算去除动态特征点的特征点集合的描述子，
    //相比orb没有在一开始提取描述子，因为包含动态特征点
    N = mvKeys.size();
    if(mvKeys.empty())
    return;
    UndistortKeyPoints();//去畸变
    ComputeStereoFromRGBD(imDepth);//计算深度
    mvpMapPoints = vector<MapPoint*>(N,static_cast<MapPoint*>(NULL));
    mvbOutlier = vector<bool>(N,false);
...
void Frame::ProcessMovingObject(const cv::Mat &imgray)
{
	...
	for (int i = 0; i < state.size(); i++)
    {
        if(state[i] != 0)
        {
            ...
            if (sum_check > limit_of_check) state[i] = 0;//2120两个9边缘灰度值之差
            if (state[i])
            {
    ...
    for (int i = 0; i < prepoint.size(); i++)
    {
        if (state[i] != 0)
        {
            ...
            // Judge outliers
            if (dd <= limit_dis_epi) continue;//1.0
